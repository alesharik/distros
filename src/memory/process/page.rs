...
use crate::memory::page_table::{map, translate, unmap, P3PageTable};
use alloc::vec::Vec;
...
impl PageAllocator {
    ...
    pub fn allocate(&mut self, pages: u64) -> Option<VirtAddr> {
        ...
        let start_page = Page::<Size4KiB>::containing_address(VirtAddr::new(
            frame.start_address().as_u64() + PROCESS_HEAP_START,
        ));
        let end_page = Page::<Size4KiB>::containing_address(VirtAddr::new(
            frame.start_address().as_u64() + PROCESS_HEAP_START + pages * Size4KiB::SIZE,
        ));
        for (idx, p) in Page::range_inclusive(start_page, end_page).enumerate() {
            let phys = unsafe {
                PhysFrame::<Size4KiB>::from_start_address_unchecked(PhysAddr::new_truncate(
                    frame.start_address().as_u64() + idx as u64 * Size4KiB::SIZE,
                ))
            };
    ...
    pub fn deallocate(&mut self, addr: VirtAddr) {
        ...
        let start_page = Page::<Size4KiB>::containing_address(VirtAddr::new(
            frame.as_u64() + PROCESS_HEAP_START,
        ));
        let end_page = Page::<Size4KiB>::containing_address(VirtAddr::new(
            frame.as_u64() + PROCESS_HEAP_START + pages * Size4KiB::SIZE,
        ));
