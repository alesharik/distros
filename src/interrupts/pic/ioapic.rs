use x2apic::ioapic::{IoApic, IrqMode, IrqFlags};
...
impl IoApicManager {
    ...
    fn init(&mut self) {
        for apic in self.apics.iter_mut() {
            unsafe {
                apic.handler.init((crate::interrupts::INT_IOAPIC_OFFSET + apic.global_system_interrupt_base as usize) as u8)
            }
        }
    }
    ...
    fn map_isr_irq(&mut self, isr: u8, dest: u32) -> usize {
        ...
        let apic = self.apics.iter_mut()
            .filter(|holder| holder.in_range(*global_irq))
            .next();
        if let Some(apic) = apic {
            unsafe {
                apic.handler.enable_irq(*global_irq as u8, dest, IrqMode::Fixed, IrqFlags::empty())
            }
