...
impl FlowTree {
    ...
    pub fn put<T: Message + 'static>(&mut self, path: &str, item: FlowTreeEndpoint) -> Result<()> {
        ...
        for part in parts {
            ...
            current = if let FlowTreeNode::Branch(b) = node {
                b
            } else {
                unreachable!()
            };
        ...
        if let Err(mut error) = current.nodes.try_insert(
            last.to_owned().to_owned(),
            FlowTreeNode::Endpoint(item.clone()),
        ) {
